...
#                 # print the predicted summary and gt user summaries used for calculating F1Score
#                 print("pred_summ_upsampled.shape: ", pred_summ_upsampled.shape)
#                 print("no of frames in predicted summary: ", np.sum(pred_summ_upsampled))
#                 print("user_summary_list[i].shape: ", user_summary_list[i].shape)
#                 print("picks_list[i].shape: ", picks_list[i].shape)
#                 print("video_length: ", video_length)
#                 print("n_frames_list[i] :", n_frames_list[i])
#                 # print("pred_summ_upsampled: ", pred_summ_upsampled)
#                 # print("user_summary_list[i]: ", user_summary_list[i])
...
def evaluate_videosumm(model, val_loader, args, epoch=None):
    ...
    # Create directory to save frames data
    os.makedirs("TVSum_frames", exist_ok=True)
    ...
    with torch.no_grad():
        for k, (video_list, text_list, mask_video_list, mask_text_list, \
            ...
            video_cls_label = video_cls_label.to(args.device)
            video_loc_label = video_loc_label.to(args.device)
            video_ctr_label = video_ctr_label.to(args.device)

            text_cls_label = text_cls_label.to(args.device)
            text_loc_label = text_loc_label.to(args.device)
            text_ctr_label = text_ctr_label.to(args.device)
            ...
            for i in range(batch_size):
                # Get video name from dataset
                try:
                    video_name = val_loader.dataset.get_video_name(k * batch_size + i)
                except:
                    video_name = f"tvsum_video_{k * batch_size + i}"
                
                video_length = np.sum(mask_video_bool[i])
                pred_cls = pred_cls_batch[i, mask_video_bool[i]]
                pred_bboxes = np.clip(pred_bboxes_batch[i, mask_video_bool[i]], 0, video_length).round().astype(np.int32)

                pred_cls, pred_bboxes = nms(pred_cls, pred_bboxes, args.nms_thresh)
                pred_summ, pred_summ_upsampled, pred_score, pred_score_upsampled = bbox2summary(
                    video_length, pred_cls, pred_bboxes, change_points_list[i], n_frames_list[i], 
                    n_frame_per_seg_list[i], picks_list[i], proportion=ratio_list[i], seg_score_mode='mean')
                
                eval_metric = 'max' if args.dataset == 'SumMe' else 'avg'
                
                # Use first user's summary as ground truth
                first_user_summary = user_summary_list[i][0] if user_summary_list[i].ndim > 1 else user_summary_list[i]
                
                fscore = get_summ_f1score(pred_summ_upsampled, user_summary_list[i], eval_metric=eval_metric)
                stats.update(fscore=fscore)
                
                # Convert binary arrays to frame indices
                identified_key_frames = np.where(pred_summ_upsampled == 1)[0].tolist()
                gt_key_frames = np.where(first_user_summary == 1)[0].tolist()
                
                # Save the data to a JSON file
                output_data = {
                    'video_name': video_name,
                    'identified_key_frames': identified_key_frames,
                    'gt_key_frames': gt_key_frames
                }
                
                # Save the data
                import json
                with open(os.path.join("TVSum_frames", f"{video_name}.json"), 'w') as f:
                    json.dump(output_data, f, indent=4)
                
                logger.info(f"Saved frame data for {video_name}: {len(identified_key_frames)} key frames identified, {len(gt_key_frames)} ground truth frames")
                
            if (k + 1) % args.print_freq == 0:
                logger.info(f'[Eval] Epoch: {epoch+1}/{args.max_epoch} Iter: {k+1}/{data_length} F-score: {stats.fscore:.4f}')
    
    logger.info(f"Completed saving frame data to TVSum_frames folder")
    return stats.fscore